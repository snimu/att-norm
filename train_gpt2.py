# This is copied and modified from https://github.com/karpathy/llm.c/blob/master/train_gpt2.py
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.scale_attn_weights:
            self.att_scale = nn.Parameter(torch.ones(1, config.n_head, 1, 1))  # att-shape: (B, nh, T, T)
        else:
            self.att_scale = None
        if config.scale_q_weights:
        ...
        self.qk_activ = config.qk_activ
        self.qk_norm = config.qk_norm
        self.att_activ = config.att_activ
        self.post_att_norm = config.post_att_norm
    ...
    def forward(self, x):
        ...
        if FLASH:
            ...
            # Post attention norm
            if self.post_att_norm == "rms_norm":
                y = rms_norm(y)
...
class GPTConfig:
    ...
    n_head: int = 12
    n_embd: int = 768
    qk_activ: Literal['none', 'gelu'] = 'none'
...
if __name__ == "__main__":
    ...
    if args.model[0] == "d":
        ...
        extra_config = {
            ...
            "scale_q_weights": args.scale_q_weights,
        }
        model_config = {
            "d12": GPTConfig(block_size=1024, vocab_size=50257, n_layer=12, n_head=12, n_embd=768, **extra_config),
            "d24": GPTConfig(block_size=1024, vocab_size=50257, n_layer=24, n_head=16, n_embd=1024, **extra_config),
            "d36": GPTConfig(block_size=1024, vocab_size=50257, n_layer=36, n_head=20, n_embd=1280, **extra_config),
            "d48": GPTConfig(block_size=1024, vocab_size=50257, n_layer=48, n_head=25, n_embd=1600, **extra_config),
        }[args.model]
